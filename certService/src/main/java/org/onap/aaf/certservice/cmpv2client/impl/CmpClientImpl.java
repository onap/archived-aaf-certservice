...
public class CmpClientImpl implements CmpClient {
  ...
  private List<List<X509Certificate>> checkCmpCertRepMessage(final PKIMessage respPkiMessage)
    ...
    if (Objects.nonNull(pkiBody) && pkiBody.getContent() instanceof CertRepMessage) {
      ...
      if (Objects.nonNull(certRepMessage)) {
        final CertResponse certResponse = getCertificateResponse(certRepMessage);
        try {
  ...
  private CertResponse getCertificateResponse(CertRepMessage certRepMessage) {
  ...
  private List<List<X509Certificate>> retrieveCertificates(String caName, CSRMeta csrMeta,
      PKIMessage pkiMessage, Cmpv2HttpClient cmpv2HttpClient) throws CmpClientException {
