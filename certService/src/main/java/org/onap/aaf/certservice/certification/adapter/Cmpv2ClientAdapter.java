...
public class Cmpv2ClientAdapter {
    ...
    public CertificationModel callCmpClient(CsrModel csrModel, Cmpv2Server server)
        ...
        return new CertificationModel(convertFromX509CertificateList(certificates.get(0)),
                convertFromX509CertificateList(certificates.get(1)));
    }
    ...
    private X509Certificate convertToX509Certificate(PKCS10CertificationRequest csr, PrivateKey privateKey)
        ...
        try {
            X509v3CertificateBuilder certificateGenerator = x509CertificateBuilder.build(csr, privateKey);
            ...
            return certificateFactoryProvider.generateCertificate(
    ...
    private List<String> convertFromX509CertificateList(List<X509Certificate> certificates) {
        return certificates.stream().map(this::convertFromX509Certificate).filter(cert -> !cert.isEmpty())
