...
class KeystoreTruststoreCreatorTest {
    ...
    void createKeystoreShouldCallRequiredMethods() throws PemToPKCS12ConverterException {
        ...
        final List<String> certificates = Arrays.asList(new String[]{"a", "b"});
        final int passwordLength = 24;
        final String alias = "keystore1";
        ...
        when(converter.convertKeystore(certificates, password, alias)).thenReturn(keystoreBytes);
        creator.createKeystore(certificates, passwordLength, alias);

        // then
        verify(passwordGenerator, times(1)).generate(passwordLength);
        verify(converter, times(1)).convertKeystore(certificates, password, alias);
        verify(filesCreator, times(1)).saveKeystoreData(keystoreBytes, password);
    }
    ...
    void createTruststoreShouldCallRequiredMethods() throws PemToPKCS12ConverterException {
        ...
        final List<String> certificates = Arrays.asList(new String[]{"a", "b"});
        final int passwordLength = 24;
        final String alias = "truststore1";
        ...
        creator.createTruststore(certificates, passwordLength, alias);

        // then
        verify(passwordGenerator, times(1)).generate(passwordLength);
        verify(converter, times(1)).convertTruststore(certificates, password, alias);
        verify(filesCreator, times(1)).saveTruststoreData(truststoreBytes, password);
    }
}