...
public class CsrProcedure {
    ...
    private final KeyPair keyPair = new KeyPairFactory(RSA_ENCRYPTION_ALGORITHM, KEY_SIZE).create();

    CsrProcedure(CsrConfiguration configuration) {
        this.configuration = configuration;
    }

    public String createEncodedCsr() {
        ...
        try {
            request = createPKCS10Csr(subject);
        } catch (OperatorCreationException e) {
            throw new CsrConfigurationException("Cannot generate CSR");
        }

        return decodeString(convertPKC10CsrToPem(request));
    }
    ...
    private String getOptionalParameters() {
        ...
        Optional.ofNullable(configuration.getOrganizationUnit())
                ...
                .map(ou -> optionalParameters.append(", " + ORGANIZATION_UNIT + "=").append(ou));
        Optional.ofNullable(configuration.getLocation())
                .filter(CsrProcedure::isOptionalParameterExists)
                .map(l -> optionalParameters.append(", " + LOCATION + "=").append(l));
        return optionalParameters.toString();
    }

    private PKCS10CertificationRequest createPKCS10Csr(X500Principal subject) throws OperatorCreationException {
        ContentSigner contentSigner = new JcaContentSignerBuilder(SIGN_ALGORITHM).build(keyPair.getPrivate());
        ...
        Optional.ofNullable(configuration.getSans())
                .filter(CsrProcedure::isOptionalParameterExists)
                .map(s -> builder
                        .addAttribute(PKCSObjectIdentifiers.pkcs_9_at_extensionRequest, generateSansExtension()));
    ...
    private Extensions generateSansExtension() {
